...
| `CLEANUP_BATCH_SIZE`           | Number    | Maximum rows or addresses processed by each built-in cleanup task per run. Defaults to `1000`, valid range `1-5000`. Smaller values reduce per-run D1 write pressure; larger values clear backlogs faster | `1000` |
