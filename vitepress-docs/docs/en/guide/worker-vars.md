...
| `DISABLE_ADDRESS_UPDATED_AT` | Text/JSON | Set to `true` to stop individual and user-wide address activity keep-alive updates and disable built-in manual/scheduled inactive-address cleanup; defaults to `false` | `true` |
...
> `DISABLE_ADDRESS_UPDATED_AT=true` skips individual and bulk activity updates triggered by user settings, mailbox access, sending mail, and other address activity. New addresses still receive database-initialized timestamps; password generation, changes, and resets retain their existing behavior, including their `updated_at` updates. Manual `/admin/cleanup` requests with `cleanType=inactiveAddress` return `403`; scheduled tasks skip that cleanup type and continue the others. Cleanup by creation time, other cleanup types, and administrator-defined SQL are unaffected. Setting the flag back to `false` restores the existing behavior without backfilling activity from the disabled period.
