...
| `query` | Empty | Optional email address keyword filter for the current user's bound addresses; at most 100 bytes, without `%`, `_`, or control characters (invalid queries return 400) |
