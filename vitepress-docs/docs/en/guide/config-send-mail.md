...
| Send permission and balance | Stored in the `address_sender` table and managed independently **per email address**, not granted to an entire user account |
...
3. Click **Request for Current Address**
...
2. **Manual Request**: If `DEFAULT_SEND_BALANCE = 0`, switch to the target email address in the frontend and click **Request for Current Address** to create a pending send-access record for that address
