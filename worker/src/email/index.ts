...
import { Bindings, HonoCustomType, RPCEmailMessage, ParsedEmailContext, SubdomainForwardAddressList } from "../types";
...
async function email(message: ForwardableEmailMessage, env: Bindings, ctx: ExecutionContext) {
  ...
  try {
    // 遍历 SUBDOMAIN_FORWARD_ADDRESS_LIST
    const subdomainForwardAddressList = env.SUBDOMAIN_FORWARD_ADDRESS_LIST;
    if (subdomainForwardAddressList) {
      for (const subAddressList of subdomainForwardAddressList) {
        // 类型断言，确保 subdomainForwardAddress 是 SubdomainForwardAddressList 类型
        const subdomainForwardAddress = subAddressList as SubdomainForwardAddressList;
        // 检查邮件是否匹配 domains
