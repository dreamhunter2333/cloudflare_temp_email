...
- fix: |CI| 修复 `Upstream Sync` 工作流在同步到包含 `.github/workflows/*` 变更的上游提交时因 `GITHUB_TOKEN` 缺少 `workflows: write` 权限而失败的问题；为 `sync_latest_from_upstream` 任务补充 `contents: write` 与 `workflows: write` 权限
