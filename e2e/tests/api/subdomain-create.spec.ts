...
import { TEST_DOMAIN, WORKER_URL, WORKER_URL_ENV_OFF } from '../../fixtures/test-helpers';
...
test.describe('Create Address Subdomain Match', () => {
  test('default stays exact match only when switch is not enabled', async ({ request }) => {
    await saveSubdomainMatchSetting(request, WORKER_URL, false);
  ...
  test('admin switch enables suffix subdomain match for both admin and user create APIs', async ({ request }) => {
    await saveSubdomainMatchSetting(request, WORKER_URL, true);

    const adminName = `subdomain-admin-${Date.now()}`;
    const adminRes = await request.post(`${WORKER_URL}/admin/new_address`, {
    ...
    const userRes = await request.post(`${WORKER_URL}/api/new_address`, {
    ...
    const invalidRes = await request.post(`${WORKER_URL}/admin/new_address`, {
