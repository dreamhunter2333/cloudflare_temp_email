...
- fix: |CI| Fix `Upstream Sync` workflow failures when syncing upstream commits that modify `.github/workflows/*`: the job token lacked `workflows: write`; add `contents: write` and `workflows: write` permissions to `sync_latest_from_upstream`
